...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in dpowcoin.conf or in an included conf file
        bad_conf_file_path = self.nodes[0].datadir_path / "dpowcoin_bad.conf"
        ...
        with open(self.nodes[0].datadir_path / 'dpowcoin.conf', 'a', encoding='utf-8') as conf:
        ...
        main_conf_file_path = self.nodes[0].datadir_path / "dpowcoin_main.conf"
    ...
    def test_config_file_log(self):
        ...
        # (conf_path, not node.dpowcoinconf)
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a dpowcoin.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "dpowcoin.conf" file which is ignored, because a '
        ...
        # the same dpowcoin.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/dpowcoin.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when dpowcoin.conf in the default data directory sets another datadir '
                      'and it contains a different dpowcoin.conf file that would be ignored')
        ...
        # dpowcoin.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "dpowcoin.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "dpowcoin.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/dpowcoin.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "dpowcoin.conf"
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "dpowcoin.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_dpowcoin.conf"
