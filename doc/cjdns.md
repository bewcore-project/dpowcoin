# CJDNS support in Dpowcoin Core

It is possible to run Dpowcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Dpowcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Dpowcoin network and individual nodes.
...
before it will work with your Dpowcoin Core node. This is described in steps
...
## Run Dpowcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Dpowcoin Core
...
When enabled, this option tells Dpowcoin Core that it is running in an
...
IPv6 local network. This helps Dpowcoin Core perform better address management:
...
CJDNS support was added to Dpowcoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `dpowcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/dpowcore-project/dpowcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Dpowcoin Core

There are several ways to see your CJDNS address in Dpowcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `dpowcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `dpowcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `dpowcoin-cli help getnodeaddresses` for details.
