# User-space, Statically Defined Tracing (USDT) for Dpowcoin Core

Dpowcoin Core includes statically defined tracepoints to allow for more
...
**Note**: Dpowcoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Dpowcoin Core
...
Multiple tools can list the available tracepoints in a `dpowcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Dpowcoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/dpowcoind
stap net        outbound_message 0x0000000000107c05 /src/dpowcoind
stap validation block_connected  0x00000000002fb10c /src/dpowcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Dpowcoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/dpowcoind | grep NT_STAPSDT -A 4 -B 2
