...
configuration options and toggled while dpowcoind is running with the `logging`
RPC.  For instance, launching dpowcoind with `-debug` or `-debug=1` will turn on
...
with "play dpowcoins" on a test network.
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run dpowcoind.

You can then use the `-debug=lock` configuration option at dpowcoind startup or
`dpowcoin-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `dpowcoin-cli logging [] '["lock"]'`.
...
([`valgrind.supp`](https://github.com/dpowcore-project/dpowcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_dpowcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_dpowcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/dpowcoind -printtoconsole
...
# A coverage report will now be accessible at `./test_dpowcoin.coverage/index.html`,
...
To profile a running dpowcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep dpowcoind` -- sleep 60
```
...
- Net threads:
  ...
  - [ThreadSocketHandler (`b-net`)](https://doxygen.bitcoincore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 42003.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `dpowcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `dpowcoin-cli` and the GUI debug console use this table to determine how to
