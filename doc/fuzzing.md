# Fuzzing Dpowcoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Dpowcoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/dpowcore-project/dpowcoin
$ cd dpowcoin/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/dpowcore-project/dpowcoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
## Overview of Dpowcoin Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/bitcoin_report.pdf) includes an external overview of the status of Dpowcoin Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.

## Fuzzing harnesses and output

[`process_message`](https://github.com/dpowcore-project/dpowcoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/dpowcore-project/dpowcoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/dpowcore-project/dpowcoin/tree/master/src/test/fuzz).
...
It is possible to specify `dpowcoind` arguments to the `fuzz` executable.
...
Every single pull request submitted against the Dpowcoin Core repo is automatically tested against all inputs in the [`bitcoin-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Dpowcoin Core more robust.
...
with certain assembly code from Dpowcoin Core's code. See [developer notes on sanitizers](https://github.com/dpowcore-project/dpowcoin/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Dpowcoin Core using afl++

## Quickstart guide

To quickly get started fuzzing Dpowcoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Dpowcoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Dpowcoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Dpowcoin Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Dpowcoin
Core without having to write any custom fuzzing harness. The `dpowcoind` server
...
```sh
$ mkdir dpowcoin-honggfuzz-p2p/
$ cd dpowcoin-honggfuzz-p2p/
$ git clone https://github.com/dpowcore-project/dpowcoin
$ cd dpowcoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
EOF
$ make -C src/ dpowcoind
$ mkdir -p inputs/
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/dpowcoind -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
                       -nodebuglogfile -bind=127.0.0.1:18444 -logthreadnames \
                       -debug
```

# Fuzzing Dpowcoin Core using Eclipser (v1.x)

## Quickstart guide

To quickly get started fuzzing Dpowcoin Core using [Eclipser v1.x](https://github.com/SoftSec-KAIST/Eclipser/tree/v1.x):
...
Dpowcoin Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/bitcoin-core)
...
because Dpowcoin is a live P2P network, and not just standalone local software,
