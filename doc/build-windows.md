...
Below are some notes on how to build Dpowcoin Core for Windows.

The options known to work for building Dpowcoin Core on Windows are:
...
Note that for WSL the Dpowcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/dpowcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\dpowcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/dpowcoin
