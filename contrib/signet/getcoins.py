...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing dpowcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='dpowcoin-cli', help='dpowcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Dpowcoin address to which the faucet should send')
...
parser.add_argument('dpowcoin_cli_args', nargs='*', help='Arguments to pass on to dpowcoin-cli (default: -signet)')

args = parser.parse_args()

if args.dpowcoin_cli_args == []:
    args.dpowcoin_cli_args = ['-signet']


def dpowcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.dpowcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = dpowcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = dpowcoin_cli(['getnewaddress', 'faucet', 'bech32'])
