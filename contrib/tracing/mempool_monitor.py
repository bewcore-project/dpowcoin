...
""" Example logging Dpowcoin Core mempool events using the mempool:added,
...
def main(dpowcoind_path):
    dpowcoind_with_usdts = USDT(path=str(dpowcoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    dpowcoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    dpowcoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    dpowcoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    dpowcoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[dpowcoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/dpowcoind")
        exit(1)
