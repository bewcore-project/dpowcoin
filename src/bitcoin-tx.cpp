...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " dpowcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  dpowcoin-tx [options] <hex-tx> [commands]  Update hex-encoded dpowcoin transaction\n"
                "or:     dpowcoin-tx [options] -create [commands]   Create hex-encoded dpowcoin transaction\n"
                "\n";
